...
class ChatbotApp:
    ...
    def __init__(self):
        ...
        if not api_key:
            st.error(f"OpenAI API key not found. Please add {config.get('api_key_env_var', 'OPENAI_API_KEY_TEG')} to your config/.env file.")
            st.stop()
    ...
    def display_sidebar(self):
        ...
        with st.sidebar:
            ...
            model = st.selectbox(
                "Select model",
                available_models,
                index=available_models.index(st.session_state.openai_model) if st.session_state.openai_model in available_models else 0
            )
